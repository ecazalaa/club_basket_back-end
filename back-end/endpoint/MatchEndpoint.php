...
switch($method) {
    case 'GET':
        ...
        if (isset($_GET['cle']) && isset($_GET['critere'])) {
            ...
            $criteres_valides = ['Id_Match', 'M_date', 'nom_adversaire', 'lieu'];
    ...
    case 'PUT':
        ...
        // Modification du résultat uniquement
        if (isset($data['resultat']) && !isset($data['date']) && !isset($data['adversaire']) && !isset($data['lieu'])) {
        ...
        // Modification complète du match
        elseif (isset($data['date']) && isset($data['adversaire']) && isset($data['lieu'])) {
            $match = new MatchBasket($data['date'], $data['adversaire'], $data['lieu']);
            $match->setIdMatch($_GET['id']);
